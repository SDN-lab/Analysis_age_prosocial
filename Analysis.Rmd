...
df.totals <- lapply(df.list, function(df) {df %>% dplyr::select("age", "Adjusted_age", "Contct4M", "generosity2", "generosity3", "generosity_sum", "generosity_nat_bias", starts_with("mcoop"), "PoliticId", "moral_circle", "Ladder", "risk1", ends_with("_average"), ends_with("_sum"), "SelfEsteem", "x2018GNI", contains("death"), contains("cases"), "date_count")})
...
for (c in cntry.list$ISO3) {
  ...
  # run linear model for each outcome & extract parameters
    ...
    country.results$represent[country.results$code == c] = cdf$REPRESENT[1]
}
...
uk.cors <- sapply(columns, function(col) {
  ...
  colnames(p2s) <- "part2"
  colcor <- rcorr(p1s$part1, p2s$part2, type= "pearson")
  colr.p <- colcor$r[1,2]
  colcor <- rcorr(p1s$part1, p2s$part2, type= "spearman")
  colr.s <- colcor$r[1,2]
