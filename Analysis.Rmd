...
df <- read.csv("data/Person_level_score_revised_Jan_2021_totals_country_data.csv", sep = ",", header = TRUE, na.strings = "NaN") # full data
...
df <- df %>% 
  ...
  mutate(AnalysisGroupF = factor(AnalysisGroup,
         levels = c(1,2),
         labels = c("S1 (n = 23,288)", "S2 (n = 23,288)")))
...
age.labs <- c("Age (years)",
              "Age (proportion of life expectancy)")

age.names <- c("age", "age_adjusted")

dona.lab <- "Amount donated (%)"
dist.lab <- "Distancing (0-10)"
...
df.totals <- lapply(df.list, function(df) {df %>% dplyr::select("age", "Contct4M", "generosity_sum")})
...
gender.means.boot$per <- c(gender.means.boot$count[1:3] / nrow(df1), gender.means.boot$count[4:6] / nrow(df2))

# correlation between prosocial outcomes

corr1 <- rcorr(df1$Contct4M, df1$generosity_sum)
corr2 <- rcorr(df2$Contct4M, df2$generosity_sum)
...
c4.mc.risk.list <- lapply(df.list, function(df) {
  ...
  c4.mc.risk.coeff$d <- t_to_d(c4.mc.risk.coeff$t, df.err, ci = F)[["d"]]
...
c4.mc.risk.results <- c4.mc.risk.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
t.log.lad.list <- lapply(df.list, function(df) {
  ...
  t.log.lad.coeff$d <- t_to_d(t.log.lad.coeff$t, df.err, ci = F)[["d"]]
...
t.log.lad.results <- t.log.lad.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
d.log.lad.list <- lapply(df.c.list, function(df) {
  ...
  df.err <- df.err$df_error[1]
  d.log.lad.coeff$d <- t_to_d(d.log.lad.coeff$t, df.err, ci = F)[["d"]]
...
d.log.lad.results <- d.log.lad.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
d.log.lad.n.results <- d.log.lad.n.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
d.log.lad.i.results <- d.log.lad.i.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=12}
...
for (av in 1:length(age.vars)) {
  ...
  # create multi-panel Figure 1 & save
...
fig1.a <- bind_cols(df1 %>% select(c("age_raw", "Contct4M")), df2 %>% select(c("age_raw", "Contct4M")))
colnames(fig1.a) <- c("Age_S1", "Distancing_S1", "Age_S2", "Distancing_S2")
fig1.b <- bind_cols(df1 %>% select(c("age_raw", "generosity_sum")), df2 %>% select(c("age_raw", "generosity_sum")))
colnames(fig1.b) <- c("Age_S1", "Donations_S1", "Age_S2", "Donations_S2")
fig1.c <- bind_cols(df.c1 %>% select(c("subjid", "age_raw", "donated", "charity")), df.c2 %>% select(c("subjid", "age_raw", "donated", "charity")))
colnames(fig1.c) <- c("Participant_S1", "Age_S1", "Donations_S1", "Charity_S1", "Participant_S2", "Age_S2", "Donations_S2", "Charity_S2")

fig1 <- list(fig1.a, fig1.b, fig1.c)
names(fig1) <- c("Fig1a", "Fig1b", "Fig1c")
if (writecsvs == 1) {writexl::write_xlsx(fig1, "source_data/Cutler_SourceData_Fig1.xlsx")}
...
# standardise continuous variables for models with income standardised for each country separately
...
d.log.obw.coeff <- model_parameters(d.log.obw, df_method = "satterthwaite", standardize = "refit")
df.err <- model_parameters(d.log.obw)
df.err <- df.err$df_error[1]
d.log.obw.coeff$d <- t_to_d(d.log.obw.coeff$t, df.err, ci = F)[["d"]]
...
plot.age.charity <- ggplot(df.obw.c, aes(x = age_raw, y = donated)) + 
  ...
  geom_smooth(aes(group = charity, color = charity, fill = charity, linetype = charity), method = "lm", formula = y ~ x, alpha = 0.3, size = 0.75, show.legend = FALSE) + 
  geom_smooth(aes(group = charity, color = charity, fill = charity, linetype = charity), method = "lm", formula = y ~ x, alpha = 0, size = 0.75) +
  scale_x_continuous(name = age.labs[1]) +
  scale_y_continuous(name = dona.lab) + 
  scale_fill_manual(values = dotcols2, labels = char.labs, name = char.lab) + 
  scale_color_manual(values = dotcols2, labels = char.labs, name = char.lab) + 
  scale_linetype_manual(values = c("solid", "twodash"), labels = char.labs, name = char.lab) +
...
countrycols <- c("code", "region", "distancing", "national", "international", "bias", "n", "gni", "sd_total_deaths", "sd_total_cases", "sd_deaths_slope", "sd_cases_slope", "represent")
...
for (c in cntry.list$ISO3) {
  ...
  # run linear model for each outcome & extract parameters
    cntry.risk.d <- model_parameters(lm(Contct4M ~ sex + risk1 + age, data = cdf), standardize = "refit")
    cntry.lad.n <- model_parameters(lm(generosity2_logit ~ sex + Ladder + age, data = cdf), standardize = "refit")
    cntry.lad.i <- model_parameters(lm(generosity3_logit ~ sex + Ladder + age, data = cdf), standardize = "refit")
    cntry.lad.b <- model_parameters(lm(generosity_nat_bias ~ sex + Ladder + age, data = cdf), standardize = "refit")
...
# create a map figure for each outcome (distancing, national giving, and international giving) across the world and just europe

world.distance <- ggplot(df.world, aes(long, lat, group = group)) +
  geom_polygon(aes(fill = distancing), color = "white", size = .1) +
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-.35, .35), na.value = "#ECECEC", guide = NULL) +
  theme_void() +
...
europe.distance <- ggplot(df.europe, aes(long, lat, group = group)) +
  geom_polygon(aes(fill = distancing), color = "white", size = .1) +
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-.35, .35), breaks = c(-0.2, 0, 0.2), na.value = "#ECECEC", name = expression(beta)) +
  theme_void() +
...
world.national <- ggplot(df.world, aes(long, lat, group = group)) +
  ...
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-.28, .28), na.value = "#ECECEC", guide = NULL) +
  theme_void() +
  draw_plot_label("National donations", x = -180, y = -20, hjust = 0, vjust = 0, angle = 90, size = axtitle, fontface = "plain")

europe.national <- ggplot(df.europe, aes(long, lat, group = group)) +
  geom_polygon(aes(fill = national), color = "white", size = .1) +
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-.28, .28), breaks = c(-0.2, 0, 0.2), na.value = "#ECECEC", name = expression(beta)) +
  theme_void() +
  guides(fill = guide_colorbar(title.hjust = 0.05)) + 
  theme(legend.text = element_text(size = axtext),
        legend.title = element_text(size = axtitle))
...
world.inter <- ggplot(df.world, aes(long, lat, group = group)) +
  ...
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-.28, .28), na.value = "#ECECEC", guide = NULL) +
  theme_void() +
  draw_plot_label("International donations", x = -180, y = -35, hjust = 0, vjust = 0, angle = 90, size = axtitle, fontface = "plain")
...
# create multi-panel Figure 2 & save
...
fig2.a <- df.world %>% select(c("region", "distancing")) %>% distinct()
colnames(fig2.a) <- c("Country", "Distancing_beta")
fig2.b <- df.world %>% select(c("region", "national")) %>% distinct()
colnames(fig2.b) <- c("Country", "National_donations_beta")
fig2.c <- df.world %>% select(c("region", "international")) %>% distinct()
colnames(fig2.c) <- c("Country", "International_donations_beta")

fig2 <- list(fig2.a, fig2.b, fig2.c)
names(fig2) <- c("Fig2a", "Fig2b", "Fig2c")
if (writecsvs == 1) {writexl::write_xlsx(fig2, "source_data/Cutler_SourceData_Fig2.xlsx")}
...
d.log.lad.list.slope.world.inter <- lapply(df.c.list, function(df) {
  ...
  d.log.lad.clf.inter1 <- lmer(donated_logit ~ sex + Ladder +
                                 age*charity +
                                 ...
                                 (1 | Country) + (0 + Ladder | Country) + (0 + age | Country),
                               data = df, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  d.log.lad.clf.inter2 <- lmer(donated_logit ~ sex + Ladder +
                                 age*charity +
                                 age*world_total_deaths + charity*world_total_deaths +
                                 age*charity*total_deaths +
                                 age*charity*rolling_deaths_slope +
...
country.cor.max <- max(max(abs(country.cor.list[[1]][country.cor.list[[1]] != 1])), 
                       max(abs(country.cor.list[[2]][country.cor.list[[2]] != 1])))
...
c4.mc.risk.clf.list <- lapply(df.list, function(df) {
  ...
  df.err <- df.err$df_error[1]
  c4.mc.risk.clf.coeff$d <- t_to_d(c4.mc.risk.clf.coeff$t, df.err, ci = F)[["d"]] 
...
c4.mc.risk.clf.results <- c4.mc.risk.clf.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
d.log.lad.clf.list <- lapply(df.c.list, function(df) {
  d.log.lad.clf <- lmer(donated_logit ~ sex + Ladder +
                                 age*charity +
                                 age*world_total_deaths + charity*world_total_deaths +
                                 age*charity*total_deaths +
                                 age*charity*rolling_deaths_slope +
...
d.log.lad.clf.results <- d.log.lad.clf.results[,c(2:3,5:9,11, 13:14,16:20,22)]
...
rownames(d.log.lad.clf.results) <- c("Intercept", "Gender (F > M)", "Subjective wealth", "Age", "Charity (I > N)", "Deaths - DTW", "Deaths - DTC", "Deaths - DRC", "Country wealth", "Age * Charity", "Age * DTW", "Charity * DTW", "Age * DTC", "Charity * DTC", "Age * DRC", "Charity * DRC", "Age * CW", "Charity * CW", "Age * Charity * DTC", "Age * Charity * DRC")
...
# multiply standard deviations by model coefficients
...
nfact <- 6 # determined with parallel analysis (commented out below)

# run factor analysis on each subsample
...
fa.list <- lapply(df.list, function(df) {
  ...
  df.fa <- df.totals %>% 
    ...
    drop_na
  # print(nrow(df.fa)) # sample size with data for factor analysis
  df.fa$moral_circle <- scales::rescale(df.fa$moral_circle, to = c(0,10)) # rescale to match other measures
  
  # print(fa.parallel(df.fa, fa="both", n.iter=100, main="Scree plots with parallel analysis"))
...
# create dataframes for each sample with all columns need for factor analysis correlations & plots
...
# colours and labels for plot

fact.pal <- c("#CA4640", "#5DA2D2", "#9A66D3", "#EAA445", "#B5C347", "#F6CD53")

fact.lines <- c(2,3,1,4,5,6)

fact.lab <- "Factor score (0-10)"
fact.leg <- "Trait factor:"

graph.labs= c("<span style = 'font-size:20pt;color:#CA4640;'>**_Positive traits_**</span><br><br>
              <span style = 'font-size:16pt;'>
              Optimism<br><br>
              Wellbeing<br><br>
              Self-esteem<br><br>
              Social belonging<br><br>
              Self-control<br><br>
              </span>",
              "<span style = 'font-size:20pt;color:#5DA2D2;'>**_Negative traits_**</span><br><br>
              <span style = 'font-size:16pt;'>
              Open-mindedness (-)<br><br>
              Narcissism<br><br>
              </span>",
              "<span style = 'font-size:20pt;color:#9A66D3;'>**_Ingroup preference_**</span><br><br>
              <span style = 'font-size:16pt;'>
              Collective narcissism<br><br>
              National identity<br><br>
              Political ideology<br><br>
              </span>",
              "<span style = 'font-size:20pt;color:#EAA445;'>**_Interpersonal morality_**</span><br><br>
              <span style = 'font-size:16pt;'>
              MaC - heroism<br><br>
              MaC - reciprocity<br><br>
              MaC - group<br><br>
              MaC - family<br><br>
              MaC - deference<br><br>
              </span>",
              "<span style = 'font-size:20pt;color:#B5C347;'>**_Material morality_**</span><br><br>
              <span style = 'font-size:16pt;'>
              MaC - property<br><br>
              MaC - fairness<br><br>
              </span>",
              "<span style = 'font-size:20pt;color:#F6CD53;'>**_General morality_**</span><br><br>
              <span style = 'font-size:16pt;'>
              Moral identity<br><br>
              Moral circle<br><br>
              </span>"
)
...
# create long version of data with a row for each factor
...
df.f <- bind_rows(df1.fa, df2.fa) %>%
  ...
  mutate(factor = factor(factors,
                         levels = c(4,3,5,1,2,6),
                         labels = c("Positive traits",
                                    "Negative traits",
                                    "Ingroup preference",
  ...
  mutate(AnalysisGroupF = factor(AnalysisGroup,
         levels = c(1,2),
         labels = c("S1 (n = 22,253)", "S2 (n = 22,275)"))) %>% 
  dplyr::rename("score" = "value") %>% 
  add_column(scaled = NA)

# rescale for plotting on same scale as measures

df.f$scaled[df.f$AnalysisGroupF == "S1 (n = 22,253)"] <- scales::rescale(df.f$score[df.f$AnalysisGroupF == "S1 (n = 22,253)"], to = c(0,10))
df.f$scaled[df.f$AnalysisGroupF == "S2 (n = 22,275)"] <- scales::rescale(df.f$score[df.f$AnalysisGroupF == "S2 (n = 22,275)"], to = c(0,10))

# create plot for each measure

plot.factor.age <- ggplot(df.f, aes(x = age, y = scaled)) +
  geom_smooth(aes(group = factor, color = factor, fill = factor, linetype = factor), 
              method = "lm", formula = y ~ x, alpha = 0.1, size = 0.75, show.legend = FALSE) +
  geom_smooth(aes(group = factor, color = factor, fill = factor, linetype = factor), 
              method = "lm", formula = y ~ x, alpha = 0, size = 0.75) +
  scale_x_continuous(name = age.labs[1]) +
  scale_y_continuous(name = fact.lab, labels = scales::number_format(accuracy = 0.01), n.breaks = 6) +
  scale_fill_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_color_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_linetype_manual(values = fact.lines, labels = graph.labs, name = fact.leg) +
  theme_classic() +
  theme(legend.title = element_markdown(face = "bold", size = axtitle), 
        legend.text = element_text(size = axtext-4), legend.key.size = unit(3,"line")) +
  theme(axis.text = element_markdown(size = axtext),
        axis.title.x = element_markdown(size = axtitle),
        axis.title.y = element_markdown(size = axtitle)) +
  guides(shape = guide_legend(override.aes = list(size = 2))) +
  ...
  theme(strip.background = element_blank(), panel.spacing = unit(2, "lines"), 
        strip.text.x = element_markdown(size = axtitle, face = "bold"))
...
plot.factor.distance <- ggplot(df.f, aes(x = Contct4M, y = scaled)) +
  ...
  geom_smooth(aes(group = factor, color = factor, fill = factor, linetype = factor), 
              method = "lm", formula = y ~ x, alpha = 0, size = 0.75) +
  scale_x_continuous(name = dist.lab, breaks = c(0, 2.5, 5, 7.5, 10), labels = c("0", "2.5", "5", "7.5", "10") ) +
  scale_y_continuous(name = fact.lab, labels = scales::number_format(accuracy = 0.01), n.breaks = 6) +
  scale_fill_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_color_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_linetype_manual(values = fact.lines, labels = graph.labs, name = fact.leg) +
  ...
  facet_rep_wrap(~ AnalysisGroupF, scales = "fixed", repeat.tick.labels = 'all') + 
  theme(strip.background = element_blank(), panel.spacing = unit(2, "lines"), 
        strip.text.x = element_markdown(size = axtitle, colour = "white"))
...
plot.factor.national <- ggplot(df.f, aes(x = generosity2, y = scaled)) +
  ...
  geom_smooth(aes(group = factor, color = factor, fill = factor, linetype = factor), 
              method = "lm", formula = y ~ x, alpha = 0, size = 0.75) +
  scale_x_continuous(name = "National donations (%)") +
  scale_y_continuous(name = fact.lab, labels = scales::number_format(accuracy = 0.01), n.breaks = 6) +
  scale_fill_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_color_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_linetype_manual(values = fact.lines, labels = graph.labs, name = fact.leg) +
...
plot.factor.international <- ggplot(df.f, aes(x = generosity3, y = scaled)) +
  ...
  geom_smooth(aes(group = factor, color = factor, fill = factor, linetype = factor), 
              method = "lm", formula = y ~ x, alpha = 0, size = 0.75) +
  scale_x_continuous(name = "International donations (%)") +
  scale_y_continuous(name = fact.lab, labels = scales::number_format(accuracy = 0.01), n.breaks = 6) +
  scale_fill_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_color_manual(values = fact.pal, labels = graph.labs, name = fact.leg) +
  scale_linetype_manual(values = fact.lines, labels = graph.labs, name = fact.leg) +
...
# create multi-panel Figure 3 & save

filename <- paste0("plots/Correlations_factors.tiff")
if (saveplots == 1) {tiff(filename, units="in", res = resolution, width = plotW * 3.2, height = plotW * 4)}
...
factorlabels <- c("Positive_traits",
                  "Negative_traits",
                  "Ingroup_preference",
                  "Interpersonal_morality",
                  "Material_morality",
                  "General_morality")
fig3.a1 <- df1.fa %>% select(c("age", "f4","f3","f5","f1","f2","f6")) 
fig3.a1[,c(2:7)] <- lapply(fig3.a1[,c(2:7)], scales::rescale, to = c(1,10))
fig3.a2 <- df2.fa %>% select(c("age", "f4","f3","f5","f1","f2","f6"))
fig3.a2[,c(2:7)] <- lapply(fig3.a2[,c(2:7)], scales::rescale, to = c(1,10))
colnames(fig3.a1) <- c("Age", factorlabels)
colnames(fig3.a2) <- c("Age", factorlabels)

fig3.b1 <- df1.fa %>% select(c("Contct4M", "f4","f3","f5","f1","f2","f6"))
fig3.b1[,c(2:7)] <- lapply(fig3.b1[,c(2:7)], scales::rescale, to = c(1,10))
fig3.b2 <- df2.fa %>% select(c("Contct4M", "f4","f3","f5","f1","f2","f6"))
fig3.b2[,c(2:7)] <- lapply(fig3.b2[,c(2:7)], scales::rescale, to = c(1,10))
colnames(fig3.b1) <- c("Distancing", factorlabels)
colnames(fig3.b2) <- c("Distancing", factorlabels)

fig3.c1 <- df1.fa %>% select(c("generosity2", "f4","f3","f5","f1","f2","f6"))
fig3.c1[,c(2:7)] <- lapply(fig3.c1[,c(2:7)], scales::rescale, to = c(1,10))
fig3.c2 <- df2.fa %>% select(c("generosity2", "f4","f3","f5","f1","f2","f6"))
fig3.c2[,c(2:7)] <- lapply(fig3.c2[,c(2:7)], scales::rescale, to = c(1,10))
colnames(fig3.c1) <- c("National_donations", factorlabels)
colnames(fig3.c2) <- c("National_donations", factorlabels)

fig3.d1 <- df1.fa %>% select(c("generosity3", "f4","f3","f5","f1","f2","f6"))
fig3.d1[,c(2:7)] <- lapply(fig3.d1[,c(2:7)], scales::rescale, to = c(1,10))
fig3.d2 <- df2.fa %>% select(c("generosity3", "f4","f3","f5","f1","f2","f6"))
fig3.d2[,c(2:7)] <- lapply(fig3.d2[,c(2:7)], scales::rescale, to = c(1,10))
colnames(fig3.d1) <- c("International_donations", factorlabels)
colnames(fig3.d2) <- c("International_donations", factorlabels)

fig3 <- list(fig3.a1, fig3.a2, fig3.b1, fig3.b2, fig3.c1, fig3.c2, fig3.d1, fig3.d2)
names(fig3) <- c("Fig3a_S1", "Fig3a_S2", "Fig3b_S1", "Fig3b_S2", "Fig3c_S1", "Fig3c_S2", "Fig3d_S1", "Fig3d_S2")
if (writecsvs == 1) {writexl::write_xlsx(fig3, "source_data/Cutler_SourceData_Fig3.xlsx")}
...
# create list of factor analysis output to apply over
...
combine.fa.corr <- round(combine.fa.corr, 2)
...
# create dataframes for each sample with all columns need for SEMs

df1.fa <- df1 %>% dplyr::select("age", "Adjusted_age", "generosity1", "generosity2", "generosity3", "generosity_sum", "generosity_nat_bias", "Contct4M", starts_with("mcoop"), "PoliticId", "moral_circle", ends_with("_average"), ends_with("_sum"), "SelfEsteem", "Ladder", "total_deaths", "rolling_deaths_slope", "x2018GNI", "AnalysisGroup", "generosity2_logit", "generosity3_logit", "risk1", "Ladder") %>%
  drop_na(-contains(c("Adjusted_age", "generosity", "mcoop_average", "risk_average", "ctheory", "contact", "hygiene", "psupport", "Contct4M", "Ladder", "total_deaths", "rolling_deaths_slope", "x2018GNI", "risk1", "Ladder"))) %>% 
  add_column(intmor = fa.list[[1]][["scores"]][,"MR1"],
             matmor = fa.list[[1]][["scores"]][,"MR2"],
             negtrt = (fa.list[[1]][["scores"]][,"MR3"] * -1), # flip sign of scores on this factor           
             postrt = fa.list[[1]][["scores"]][,"MR4"], 
             ingpre = fa.list[[1]][["scores"]][,"MR5"], 
             genmor = fa.list[[1]][["scores"]][,"MR6"], 
             .after = "Contct4M") %>%
  drop_na(c("risk1", "Ladder"))

df2.fa <- df2 %>% dplyr::select("age", "Adjusted_age", "generosity1", "generosity2", "generosity3", "generosity_sum", "generosity_nat_bias", "Contct4M", starts_with("mcoop"), "PoliticId", "moral_circle", ends_with("_average"), ends_with("_sum"), "SelfEsteem", "Ladder", "total_deaths", "rolling_deaths_slope", "x2018GNI", "AnalysisGroup", "generosity2_logit", "generosity3_logit", "risk1", "Ladder") %>%
  drop_na(-contains(c("Adjusted_age", "generosity", "mcoop_average", "risk_average", "ctheory", "contact", "hygiene", "psupport", "Contct4M", "Ladder", "total_deaths", "rolling_deaths_slope", "x2018GNI", "risk1", "Ladder"))) %>% 
  add_column(intmor = fa.list[[2]][["scores"]][,"MR1"],
             matmor = fa.list[[2]][["scores"]][,"MR2"],
             negtrt = (fa.list[[2]][["scores"]][,"MR3"] * -1), # flip sign of scores on this factor
             postrt = fa.list[[2]][["scores"]][,"MR4"],
             ingpre = fa.list[[2]][["scores"]][,"MR5"],
             genmor = fa.list[[2]][["scores"]][,"MR6"],
             .after = "Contct4M") %>%
  drop_na(c("risk1", "Ladder"))

# create list of dataframes to apply over

df.fa.list <- list(df1.fa, df2.fa)

# standardise continuous variables for models
df.fa.list <- lapply(df.fa.list, function(df) {
  df[,c("age", "generosity2_logit", "generosity3_logit", "Contct4M", "intmor", "matmor", "negtrt", "postrt", "ingpre", "genmor", "risk1", "Ladder")] <- scale(df[,c("age", "generosity2_logit", "generosity3_logit", "Contct4M", "intmor", "matmor", "negtrt", "postrt", "ingpre", "genmor", "risk1", "Ladder")])
  return(df)
})

# run structural equation models on each subsample separately

sem.results.list <- lapply(df.fa.list, function(df) {
  
  # specify model of distancing with age, the 6 factors and perceived risk
  model.dist <- '
  Contct4M ~ b1*postrt + b2*negtrt + b3*ingpre + b4*intmor + b5*matmor + b6*genmor + c*age + d*risk1
  postrt ~ a1*age
  negtrt ~ a2*age
  ingpre ~ a3*age
  intmor ~ a4*age
  ...
  matmor~~genmor'
  # run model
  sem.dist <- sem(model.dist, data = df)
  # extract parameters
  # summary(sem.dist, standardized = TRUE)
  sem.dist.results <- parameterEstimates(sem.dist) %>% 
    select(c(lhs:est, pvalue)) %>% 
    filter(op == "~" | op == ":=") %>% 
    add_column(.before = "lhs", "measure" = "Contct4M")
  
  # specify model of national giving with age, the 6 factors and subjective wealth
  model.nat <- '
  generosity2_logit ~ b1*postrt + b2*negtrt + b3*ingpre + b4*intmor + b5*matmor + b6*genmor + c*age + d*Ladder
  postrt ~ a1*age
  negtrt ~ a2*age
  ingpre ~ a3*age
  intmor ~ a4*age
  ...
  matmor~~genmor'
  # run model
  sem.nat <- sem(model.nat, data = df)
  # extract parameters
  # summary(sem.nat)
  sem.nat.results <- parameterEstimates(sem.nat) %>% 
    select(c(lhs:est, pvalue)) %>% 
    filter(op == "~" | op == ":=") %>% 
    add_column(.before = "lhs", "measure" = "generosity2")
  
  # specify model of national giving with age, the 6 factors and subjective wealth
  model.inter <- '
  generosity3_logit ~ b1*postrt + b2*negtrt + b3*ingpre + b4*intmor + b5*matmor + b6*genmor + c*age + d*Ladder
  postrt ~ a1*age
  negtrt ~ a2*age
  ingpre ~ a3*age
  intmor ~ a4*age
  ...
  matmor~~genmor'
  # run model
  sem.inter <- sem(model.inter, data = df)
  # extract parameters
  # summary(sem.inter)
  sem.inter.results <- parameterEstimates(sem.inter) %>% 
    select(c(lhs:est, pvalue)) %>% 
    filter(op == "~" | op == ":=") %>% 
    add_column(.before = "lhs", "measure" = "generosity3")

  # combine distancing, national giving and international giving results  
  sem.results <-  bind_rows(sem.dist.results, sem.nat.results, sem.inter.results) %>% 
  add_column(pcor = NA)
  
})

# change names for subsample 2 so unique when combine
sem.results.list[[2]] <- rename(sem.results.list[[2]], est1 = est)
sem.results.list[[2]] <- rename(sem.results.list[[2]], pvalue1 = pvalue)
sem.results.list[[2]] <- rename(sem.results.list[[2]], pcor1 = pcor)

# combine results for subsample 1 and subsample 2
sem.results <- cbind(sem.results.list[[1]], select(sem.results.list[[2]], c("est1", "pvalue1", "pcor1")))

# apply Bonferroni corrections to the p values for each category of effect in each subsample 
sem.results$pcor[grepl("a[0-9]",sem.results$label)] <- p.adjust(
  sem.results$pvalue[grepl("a[0-9]",sem.results$label)], method = "bonferroni")
sem.results$pcor[grepl("b[0-9]",sem.results$label)] <- p.adjust(
  sem.results$pvalue[grepl("b[0-9]",sem.results$label)], method = "bonferroni")
sem.results$pcor[grepl("indirect",sem.results$label)] <- p.adjust(
  sem.results$pvalue[grepl("indirect",sem.results$label)], method = "bonferroni")
sem.results$pcor[sem.results$label == "c"] <- p.adjust(
  sem.results$pvalue[sem.results$label == "c"], method = "bonferroni")
sem.results$pcor[sem.results$label == "d"] <- p.adjust(
  sem.results$pvalue[sem.results$label == "d"], method = "bonferroni")
sem.results$pcor[sem.results$label == "total"] <- p.adjust(
  sem.results$pvalue[sem.results$label == "total"], method = "bonferroni")

sem.results$pcor1[grepl("a[0-9]",sem.results$label)] <- p.adjust(
  sem.results$pvalue1[grepl("a[0-9]",sem.results$label)], method = "bonferroni")
sem.results$pcor1[grepl("b[0-9]",sem.results$label)] <- p.adjust(
  sem.results$pvalue1[grepl("b[0-9]",sem.results$label)], method = "bonferroni")
sem.results$pcor1[grepl("indirect",sem.results$label)] <- p.adjust(
  sem.results$pvalue1[grepl("indirect",sem.results$label)], method = "bonferroni")
sem.results$pcor1[sem.results$label == "c"] <- p.adjust(
  sem.results$pvalue1[sem.results$label == "c"], method = "bonferroni")
sem.results$pcor1[sem.results$label == "d"] <- p.adjust(
  sem.results$pvalue1[sem.results$label == "d"], method = "bonferroni")
sem.results$pcor1[sem.results$label == "total"] <- p.adjust(
  sem.results$pvalue1[sem.results$label == "total"], method = "bonferroni")

if (writecsvs == 1) {write.csv(x = sem.results, file = "output/SEM_results.csv")}
...
# repeat models from main analysis after removing participants in the pilot analysis (before preregistration)
...
# repeat models from main analysis using adjusted instead of raw age
...
# repeat models from main analysis including additional control
...
# standardise continuous variables for models
df.list <- lapply(df.list, function(df) {
  df[,c("generosity_sum_logit", "age", "Ladder", "Contct4M", "risk1", "health_cond")] <- scale(df[,c("generosity_sum_logit", "age", "Ladder", "Contct4M", "risk1", "health_cond")])
  return(df)
})

# model distancing (note. Contct4M is the mean of the 4-item distancing measure and higher scores always = more distancing)
c4.mc.risk.list <- lapply(df.list, function(df) {
  c4.mc.risk <- lmer(Contct4M ~ sex + risk1 + health_cond + age + (1 | Country) + (0 + risk1 | Country) + (0 + health_cond | Country) + (0 + age | Country), data = df, control = lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
  c4.mc.risk.coeff <- model_parameters(c4.mc.risk, df_method = "satterthwaite", standardize = "refit")
  df.err <- model_parameters(c4.mc.risk)
  df.err <- df.err$df_error[1]
  c4.mc.risk.coeff$d <- t_to_d(c4.mc.risk.coeff$t, df.err, ci = F)[["d"]]
...
c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p"] = pvalr(c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p"])
c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p.1"] = pvalr(c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p.1"])
rownames(c4.mc.risk.results) <- c("Intercept", "Gender (F > M)", "Perceived risk", "Health condition", "Age")
if (writecsvs == 1) {write.csv(x = c4.mc.risk.results, file = "output/Distancing_model_results_health.csv")}
...
# repeat models from main analysis without control
...
d.log.lad.list <- lapply(df.c.list, function(df) {
  ...
  d.log.lad.coeff <- model_parameters(d.log, df_method = "satterthwaite", standardize = "refit")
...
# for each date calculate the effect of age for each outcome
...
for (c in dates$date_count) {
  ...
  if (nrow(cdf) > 100) {
    ...
    date.lad.d <- model_parameters(lm(Contct4M ~ sex + risk1 + age, data = cdf), standardize = "refit")
    date.lad.n <- model_parameters(lm(generosity2_logit ~ sex + Ladder + age, data = cdf), standardize = "refit")
    date.lad.i <- model_parameters(lm(generosity3_logit ~ sex + Ladder + age, data = cdf), standardize = "refit")
...
# plot a heatmap of the effect sizes on each day
...
plot.date.prosocial <- ggplot(date.results.long, aes(x = date_count, y = outcome, fill = beta)) +
  ...
  scale_y_discrete(breaks = c("international", "national", "distancing"), labels = c("International\ndonations", "National\ndonations", "Distancing")) +
  scale_fill_gradient2(low = neg.corr.col, mid = "white", high = pos.corr.col, midpoint = 0, limits = c(-0.34, 0.34), na.value = "#ECECEC", name = expression(beta)) +
  theme_classic() +
  theme(
...
# plot the covid total deaths on each day
...
# plot the covid death rates on each day
...
# calculate the sample size for each country and combine with income data
...
# countries had dates in different format, checked manually and info included in a csv file
...
# combine all required information
...
# some countries missing exact data collection dates so mark to differentiate in plot
...
# checked these files manually - 
# no ages under 18 or over 100, no participant with data in both parts failed enter number attention check

df.uk1 <- as_tibble(df.uk1)
df.uk2 <- as_tibble(df.uk2)

# combine data from two rounds of testing

df.uk <- inner_join(df.uk1, df.uk2, by = "Prolific.ID", suffix = c("_part1", "_part2")) %>% 
  dplyr::select(starts_with(c(
    "Prolific.ID", 
    "age", "generosity_2", "generosity_3", "generosity_sum", "physical_contact_av", 
    "collective_narcis_av", "national_identity_av", "political_ideology_1", "narcissism_av", "open_mindness_av", "trait_optimism_av", "social_belonging_av", "trait_self.control_av", "psych_wellbeing_av", "self.esteem", "morality_as_cooperat", "moral_identity_av", "moral_circle"
    , "ladder", "risk_perception_av"
  )))  %>% 
  dplyr::select(-contains("cooperat_av"))

df.uk.long <- df.uk %>% pivot_longer(cols = !matches("Prolific.ID"), names_to = c(".value", "part"), names_sep = "_part")
df.uk.long$part <- factor(df.uk.long$part)

columns <- colnames(df.uk.long)
columns <- columns[3:length(columns)]

uk.cors <- sapply(columns, function(col) {
  nrows <- nrow(df.uk.long[,colnames(df.uk.long) == col])
  p1s <- df.uk.long[df.uk.long$part == 1,colnames(df.uk.long) == col]
  colnames(p1s) <- "part1"
  p2s <- df.uk.long[df.uk.long$part == 2,colnames(df.uk.long) == col]
  colnames(p2s) <- "part2"
  ps <- bind_cols(p1s, p2s)
  
  if (grepl ('average', col) | grepl ('sum', col)) {
    unit <- "average"
  } else {unit <- "single"}
  
  col.icc <- icc(ps, model = "twoway", type = "agreement", unit = unit)
  colr.i <- col.icc$value
  })
...
# repeat models from main analysis after removing student participants

df1 <- subset(full, AnalysisGroup == 1 & employ_status != 4)
df2 <- subset(full, AnalysisGroup == 2 & employ_status != 4)
...
c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p.1"] = pvalr(c4.mc.risk.results[,colnames(c4.mc.risk.results) == "p.1"])
rownames(c4.mc.risk.results) <- c("Intercept", "Gender (F > M)", "Perceived risk", "Age")
if (writecsvs == 1) {write.csv(x = c4.mc.risk.results, file = "output/Distancing_model_results_no_students.csv")}
...
t.log.lad.results[,colnames(t.log.lad.results) == "p.1"] = pvalr(t.log.lad.results[,colnames(t.log.lad.results) == "p.1"])
rownames(t.log.lad.results) <- c("Intercept", "Gender (F > M)", "Subjective wealth", "Age (linear)", "Age (quadratic)")
if (writecsvs == 1) {write.csv(x = t.log.lad.results, file = "output/Giving_sum_model_results_no_students.csv")}
...
d.log.lad.results[,colnames(d.log.lad.results) == "p.1"] = pvalr(d.log.lad.results[,colnames(d.log.lad.results) == "p.1"])
rownames(d.log.lad.results) <- c("Intercept", "Gender (F > M)", "Subjective wealth", "Age", "Charity (I > N)", "Age * Charity")
if (writecsvs == 1) {write.csv(x = d.log.lad.results, file = "output/Charity_model_results_no_students.csv")}

```

## Change in wellbeing

```{r echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.align="center", fig.width=8, fig.height=6.4}

df.uk$wellbeing_diff <- df.uk$psych_wellbeing_average_part2 - df.uk$psych_wellbeing_average_part1

wellbeing.diff.corr <- cor.test(df.uk$age_part1, df.uk$wellbeing_diff)

filename <- paste0("plots/Change_wellbeing.tiff")
if (saveplots == 1) {tiff(filename, units="in", res = resolution, width = plotW*2, height = (plotH * 1.6))}

plot.age.well <- ggplot(df.uk, aes(x = age_part1, y = wellbeing_diff)) + 
  geom_point(color = dotcols[1], shape = 21, fill = NA, alpha = 0.3, size = 1, show.legend = FALSE) + 
  geom_smooth(color = dotcols[1], fill = dotcols[1], linetype = "solid", method = "lm", formula = y ~ x, alpha = 0.3, size = 0.75, show.legend = FALSE) + 
  scale_x_continuous(name = "Age") +
  ylab(expression(italic("← decrease     ")~paste(Delta," wellbeing ")~italic("     increase→"))) +
  theme_classic() + 
  theme(axis.text.y = element_markdown(size = axtext),
